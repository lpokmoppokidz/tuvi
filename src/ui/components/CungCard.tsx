...
export const CungCard = ({ cung, onClick, className = "" }: { cung: CungDisplay, onClick: () => void, className?: string }) => (
  <div 
    onClick={onClick}
  ...
  >
    ...
    <div className="flex justify-between items-start mb-1 relative z-10">
      <span className={`text-[8px] font-display tracking-[0.2em] uppercase ${
    ...
    <div className="flex flex-col items-center justify-center flex-1 py-1 relative z-10">
      <h4 className={`text-[10px] font-display tracking-[0.15em] text-center leading-tight ${
    ...
    <div className="flex flex-col gap-1 mt-auto relative z-10">
      ...
      {(cung.chinhTinh ?? []).length > 0 && (
        <div className="flex flex-wrap justify-center gap-1">
          {cung.chinhTinh.map((star, idx) => (
            <span key={idx} className={`text-[9px] font-display leading-tight truncate ${
      ...
      {(cung.tuHoa ?? []).length > 0 && (
        <div className="flex flex-wrap justify-center gap-1">
          {cung.tuHoa.map((star, idx) => (
            <span key={idx} className="text-[7px] font-black text-celestial-gold uppercase tracking-tighter drop-shadow-[0_0_5px_rgba(212,175,55,0.5)]">
              {star}
            </span>
          ))}
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-0.5 opacity-30">
        {(cung.phuTinh ?? []).slice(0, 2).map((star, idx) => (
          <span key={idx} className="text-white/60 text-[6px] truncate max-w-full">
            • {star}
          </span>
        ))}
      </div>
    ...
    {cung.isMenh && (
      <div className="absolute -bottom-1 -right-1 opacity-10">
        <svg width="24" height="24" viewBox="0 0 24 24">
          <path d="M12 0L13.5 10.5L24 12L13.5 13.5L12 24L10.5 13.5L0 12L10.5 10.5L12 0Z" fill="currentColor" className="text-celestial-gold" />
        </svg>
      </div>
    )}
  </div>
