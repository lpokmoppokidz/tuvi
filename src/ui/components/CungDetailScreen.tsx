...
import {
  ...
  Briefcase, AlertTriangle, Users, Home, Activity, Globe, Baby, Sparkles,
} from "lucide-react";
...
interface Props { cung: CungDisplay; onBack: () => void; }

export const CUNG_ICON: Record<string, any> = {
  "Menh":       { icon: Star,      color: "text-purple-400", bg: "bg-purple-400/10" },
  "Phu The":    { icon: Heart,     color: "text-pink-400",   bg: "bg-pink-400/10"   },
  "Tai Bach":   { icon: Coins,     color: "text-yellow-400", bg: "bg-yellow-400/10" },
  "Quan Loc":   { icon: Briefcase, color: "text-blue-400",   bg: "bg-blue-400/10"   },
  "Thien Di":   { icon: Globe,     color: "text-green-400",  bg: "bg-green-400/10"  },
  "Phu Mau":    { icon: Users,     color: "text-orange-400", bg: "bg-orange-400/10" },
  "Dien Trach": { icon: Home,      color: "text-amber-400",  bg: "bg-amber-400/10"  },
  "Tat Ach":    { icon: Activity,  color: "text-red-400",    bg: "bg-red-400/10"    },
  "Tu Tuc":     { icon: Baby,      color: "text-cyan-400",   bg: "bg-cyan-400/10"   },
  "Huynh De":   { icon: Users,     color: "text-indigo-400", bg: "bg-indigo-400/10" },
  "No Boc":     { icon: Users,     color: "text-teal-400",   bg: "bg-teal-400/10"   },
  "Phuc Duc":   { icon: Star,      color: "text-violet-400", bg: "bg-violet-400/10" },
...
export const CungDetailScreen: React.FC<Props> = ({ cung, onBack }) => {
  const { t } = useTranslation();
  const [activeTab, setActiveTab] = useState<"tong_quan"|"chinh_tinh"|"phu_tinh"|"tu_hoa">("tong_quan");

  const ic  = CUNG_ICON[cung.ten] || CUNG_ICON["default"];
  const IC  = ic.icon;
